...
export ROLE_ORACLE_ADMIN=$ROLE_DEFAULT_ADMIN
export ROLE_ORACLE_ADMIN_PK=$ROLE_DEFAULT_ADMIN_PK
...
export ORACLE_MANAGER=$(task deploy -- OracleManager --constructor-args $ROLE_ORACLE_ADMIN $VALIDATOR_WHITELIST $ORACLE_WHITELIST | grep -i "deployed" | cut -d " " -f 3)
export ORACLE=$(task deploy -- Oracle --constructor-args $ROLE_ORACLE_ADMIN $ORACLE_MANAGER | grep -i "deployed" | cut -d " " -f 3)
